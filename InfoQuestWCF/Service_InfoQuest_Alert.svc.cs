...
{
  public partial class Services_InfoQuest : IService_InfoQuest_Alert
  {
    public string Alert_PendingApprovalNotifications_Automated(string userName, string password)
    {
      bool AccessValid = InfoQuest_Security.Security_WCF(System.Reflection.MethodBase.GetCurrentMethod().Name, userName, password);

      if (AccessValid == true)
      {
        Alert_PendingApprovalNotifications_Automated_ReturnMessageHandler.Clear();

        try
        {
          string EmailList_SecurityUserEmail = "";
          string EmailList_SendNotification = "";
          string SQLStringEmailList = "EXECUTE spForm_Execute_Alert_PendingApprovalNotifications_EmailList";
            ...
            using (DataTable_EmailList = new DataTable())
            {
              ...
              if (DataTable_EmailList.Rows.Count > 0)
              {
                foreach (DataRow DataRow_Row_EmailList in DataTable_EmailList.Rows)
                {
                  ...
                  string SQLStringNotification = "EXECUTE spForm_Execute_Alert_PendingApprovalNotifications_Notification @SecurityUserEmail , @SendNotification";
                  using (SqlCommand SqlCommand_Notification = new SqlCommand(SQLStringNotification))
                  {
                    ...
                    using (DataTable_Notification = new DataTable())
                    {
                      ...
                      {
                        Alert_PendingApprovalNotifications_Automated_ReturnMessageHandlers(Convert.ToString("Notification is Empty", CultureInfo.CurrentCulture));
                      }
                    }
                  }
                  ...
                  if (!string.IsNullOrEmpty(Notification))
                  {
                    ...
                    string EmailTemplate = InfoQuest_All.All_SystemEmailTemplate("85");
                    ...
                    if (EmailSend == "Yes")
                    {
                      Alert_PendingApprovalNotifications_Automated_ReturnMessageHandlers(Convert.ToString("Notification Send", CultureInfo.CurrentCulture));
                      EmailBody = "";
                    }
                    else if (EmailSend == "No")
                    {
                      Alert_PendingApprovalNotifications_Automated_ReturnMessageHandlers(Convert.ToString("Notification not Send", CultureInfo.CurrentCulture));
                      Alert_PendingApprovalNotificationsAutomated_Successful = "No";
                      EmailBody = "";
                    }
              ...
              {
                Alert_PendingApprovalNotifications_Automated_ReturnMessageHandlers(Convert.ToString("Email List is Empty", CultureInfo.CurrentCulture));
              }
            }
          }
        }
        catch (Exception Exception_Error)
        {
          ...
          {
            Alert_PendingApprovalNotifications_Automated_ReturnMessageHandlers(Convert.ToString("Notifications not Send", CultureInfo.CurrentCulture));
            Alert_PendingApprovalNotificationsAutomated_Successful = "No";
        ...
        foreach (KeyValuePair<string, string> KeyValuePair_ReturnMessage in Alert_PendingApprovalNotifications_Automated_ReturnMessageHandler)
        {
          ReturnMessage = ReturnMessage + KeyValuePair_ReturnMessage.Value + "\n";
        }

        if (Alert_PendingApprovalNotificationsAutomated_Successful == "No")
        {
          InfoQuest_WCF.WCF_SendEmail_Error(System.Reflection.MethodBase.GetCurrentMethod().Name, ReturnMessage);
        }
        //else if (Alert_PendingApprovalNotificationsAutomated_Successful == "Yes")
        //{
    ...
    string Alert_PendingApprovalNotificationsAutomated_Successful = "Yes";

    private static Dictionary<string, string> Alert_PendingApprovalNotifications_Automated_ReturnMessageHandler = new Dictionary<string, string>();
    private static void Alert_PendingApprovalNotifications_Automated_ReturnMessageHandlers(string ReturnMessage)
    {
      if (!Alert_PendingApprovalNotifications_Automated_ReturnMessageHandler.ContainsKey(ReturnMessage))
      {
        Alert_PendingApprovalNotifications_Automated_ReturnMessageHandler.Add(ReturnMessage, "Alert_PendingApprovalNotifications_Automated: " + ReturnMessage);
      }
    }
  }
}
