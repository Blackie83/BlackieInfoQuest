...
{
  public partial class Form_OccupationalHealthAudit_Summary_Print : InfoQuestWCF.Override_SystemWebUIPage
  {
    ...
    private string PageSecurity()
    {
      ...
      else
      {
        ...
        {
          SQLStringSecurity = "SELECT DISTINCT SecurityUser_UserName FROM vAdministration_SecurityAccess_Active WHERE (SecurityUser_UserName = @SecurityUser_UserName) AND (Form_Id IN ('48')) AND (SecurityRole_Id IN ('122','123') OR Facility_Id IN (SELECT Facility_Id FROM Form_OccupationalHealthAudit WHERE OHA_Id = @OHA_Id))";
        }
    ...
    {
      Button_Print.Visible = false;
      ScriptManager.RegisterStartupScript(UpdatePanel_OccupationalHealthAudit_Summary_Print, this.GetType(), "Print", "window.print()", true);
      GridView_OccupationalHealthAudit_Summary_List.DataBind();
    }
      ...
      for (int i = 0; i < GridView_OccupationalHealthAudit_Summary_List.Rows.Count; i++)
      {
        if (GridView_OccupationalHealthAudit_Summary_List.Rows[i].RowType == DataControlRowType.DataRow)
        {
          if (GridView_OccupationalHealthAudit_Summary_List.Rows[i].Cells[4].Text.IndexOf("heading", StringComparison.CurrentCulture) == 0)
          {
            ...
            GridView_OccupationalHealthAudit_Summary_List.Rows[i].Cells[0].BackColor = Color.FromName("#F2F2F2");
          ...
          if (GridView_OccupationalHealthAudit_Summary_List.Rows[i].Cells[4].Text.IndexOf("subheading", StringComparison.CurrentCulture) == 0)
          {
            for (int a = 0; a <= 3; a++)
            {
              ...
              GridView_OccupationalHealthAudit_Summary_List.Rows[i].Cells[a].BackColor = Color.FromName("#F2F2F2");
          ...
          for (int a = 0; a <= 3; a++)
          {
            ...
            GridView_OccupationalHealthAudit_Summary_List.Rows[i].Cells[a].Font.Name = "Verdana";
